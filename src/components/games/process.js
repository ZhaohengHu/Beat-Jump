...
function Ready() {
  ...
  function changeReadyBlock(index) {
    ...
    // 把对应的block样式设为不可见
    document.getElementsByClassName("ready-block")[index].style.visibility =
      "hidden";
    console.log(index);
  }
...
export function Processing() {
  const judgedResState = useSelector((state) => state.judgedRes.status);
  console.log(`Processing Component: ${judgedResState === " "}`);

  return <span>{judgedState2word[judgedResState]}</span>;
}
...
export default function Process({ className }) {
  ...
  switch (process) {
    ...
    case "ending":
      ...
      break;
    default:
      componentToRender = <div className="game-tip"></div>;
  }
  return (
    // 根据状态不同，返回四种不同组件
    <div className={className}>{componentToRender}</div>
  );
}

const judgedState2word = {
    'perfect': '完美',
    'good': '良好',
    'bad': '较差',
    'miss': 'MISS',
